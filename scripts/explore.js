...
function clean(data) {
  return data.map(d => ({
    ...d,
    views: +d.views,
    percent_traffic: +d.percent_trafic
  }));
}
...
function calculate({link}) {
  ...
  const median_views = d3.median(dailyData, d => d.views);
	...
	const max_change_percent_traffic = max_percent_traffic / median_percent_traffic;
	return {
		link,
		median_views,
...
function init() {
  ...
  const data = d3.csvParse(
	...
	withAverages.sort((a,b) => d3.descending(a.max_change_views, b.max_change_views))
